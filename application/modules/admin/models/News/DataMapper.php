...
class Admin_Model_News_DataMapper implements Zf_Model_Interface, Admin_Model_News_Interface
{
	/**
	...
	public function setDbTable($dbTable)
	{
        ...
        if (!$dbTable instanceof Zend_Db_Table_Abstract) {
        	...
        	throw new Admin_Model_News_Exception('Invalid table data gateway provided');
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			...
			$this->setDbTable('Admin_Model_DbTable_News');
		}
...
/**
	...
	public function o_fGetSelect($the_sz_Order = null, $the_a_Where = null)
	{
		try {
			...
			->select()
			->from('tbl_news', array('NEWS_Id', 'NEWS_Name_en', 'NEWS_Name_vi', 'NEWS_Description_en', 'NEWS_Description_vi', 'NEWS_Status', 'NEWS_CreatedDate'));

			if ( !is_null($the_a_Where) ) {

				foreach($the_a_Where as $sz_Field => $sz_Value)
				{
					if($sz_Field == 'NEWS_ParentId' || $sz_Field == 'NEWS_Sort' || $sz_Field == 'NEWS_Status')
					{
						if($sz_Field == 'NEWS_Status' && $sz_Value == Zf_Util_Const::STATUS_All)
						{
