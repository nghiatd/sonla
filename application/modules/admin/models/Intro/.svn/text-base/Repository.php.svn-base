...
class Admin_Model_Intro_Repository implements Zf_Model_RepositoryInterface, Admin_Model_Intro_Interface
{
	protected $_data;
	...
	public function setData($the_o_data)
	{
        ...
        if (!$the_o_data instanceof Admin_Model_Intro_DataMapper) {

        	throw new Admin_Model_Intro_Exception('Invalid data access object provided');
	...
	public function getData()
	{
		if (null === $this->_data) {
			...
			$this->setData('Admin_Model_Intro_DataMapper');
	...
	public function setMapper($the_o_Mapper)
	{
        ...
        if (!$the_o_Mapper instanceof Zf_Model_DataMapper) {
        	...
        	throw new Admin_Model_Intro_Exception('Invalid data mapper provided');
	...
	public function getMapper()
	{
		if (null === $this->_mapper) {
            ...
            $this->setMapper('Admin_Model_Intro_Mapper');
		}
	...
	public function v_fSave($the_o_Data)
	{
		...
		} catch (Admin_Model_Intro_Exception $ex) {
			throw $ex;
		}	
		return null;
	}
	...
	public function fetchRow($the_i_DateTime, $the_sz_Ip)
	{
		try {
			...
			if( $a_Row && is_array($a_Row) ) {
				...
				$a_Categories = $this->getMapper()->assign(new Admin_Model_Intro_Entity(), $a_Row);
	...
	public function fetchAll($the_sz_Where = null, $the_sz_Order = null, $the_i_Count = null, $the_i_Offset = null)
	{
		try {
			...
			foreach ( $a_Rows as $a_Row ) {
				...
				$a_Categoriess[] = $this->getMapper()->assign(new Admin_Model_Intro_Entity(), $a_Row);
	...
	public function b_fDelete($the_a_Data)
	{
		try {
			...
			$a_IntroInfo = $this->a_fGetIntroById($a_Data['INTRO_id']);
			return !$a_IntroInfo ? true:false;
	...
	public function a_fGetIntroById($the_i_Id)
	{
		try {
			...
			if ($a_Row && is_array($a_Row)) {
				$a_IntroData = $this->getMapper()->assign(new Admin_Model_Intro_Entity(), $a_Row);
				return $a_IntroData;
			}else {
				return null;
			}
	...
	public function a_fGetIntroInfo($the_sz_Key, $the_sz_Value)
	{
		try {
			...
			$a_Row = $this->getData()->a_fGetIntroInfo($the_sz_Key, $the_sz_Value);

			if( $a_Row && is_array($a_Row) ) {

				// Assign data row to Entity and get back to used
				$a_Categories = $this->getMapper()->assign(new Admin_Model_Intro_Entity(), $a_Row)->__toArray();
	...
	public function o_fGetSelect($the_sz_Order = null, $the_a_FilterWhere = null)
	{
		try {
			...
			//$a_CatParentEntity = $this->a_fGetIntroInfo($this->sz_fGetFieldMapper('category_id'), isset($the_a_FilterWhere[$this->sz_fGetFieldMapper('category_id')]) ? $the_a_FilterWhere[$this->sz_fGetFieldMapper('category_id')]: null);
