...
class Admin_Model_Tourer_Repository implements Zf_Model_RepositoryInterface, Admin_Model_Tourer_Interface
{
    protected $_data;
    ...
    public function setData($the_o_data)
    {
        ...
        if (!$the_o_data instanceof Admin_Model_Tourer_DataMapper) {

            throw new Admin_Model_Tourer_Exception('Invalid data access object provided');
        }
     ...
     * @author nhungnt
     * @since 23/04/2014
     * @return Admin_Model_Tourer_DataMapper
     */
    public function getData()
    {
        if (null === $this->_data) {

            $this->setData('Admin_Model_Tourer_DataMapper');
        }
    ...
    public function setMapper($the_o_Mapper)
    {
        ...
        if (!$the_o_Mapper instanceof Zf_Model_DataMapper) {
            ...
            throw new Admin_Model_Tourer_Exception('Invalid data mapper provided');
        }
    ...
    public function getMapper()
    {
        if (null === $this->_mapper) {
            ...
            $this->setMapper('Admin_Model_Tourer_Mapper');
        }
    ...
    public function v_fSave($the_o_Data)
    {
        ...
        } catch (Admin_Model_Tourer_Exception $ex) {
    ...
    public function a_fGetTourerInfo($the_sz_Key, $the_sz_Value)
    {
        try
        {
            $a_Row = $this->getData()->a_fGetTourerInfo($the_sz_Key, $the_sz_Value);

            if( $a_Row && is_array($a_Row) ) {

                // Assign data row to Entity and get back to used
                $a_Categories = $this->getMapper()->assign(new Admin_Model_Tourer_Entity(), $a_Row)->__toArray();
    ...
    public function b_fDelete($the_o_Data)
    {
        try
        {
            ...
            $a_TourerInfo = $this->a_fGetById($a_Data['TOURER_id']);

            return !$a_TourerInfo ? true : false;
    ...
    public function a_fGetById($the_i_Id)
    {
        try
        {
            ...
            if( $a_Row && is_array($a_Row) )
            {
                $a_Categories = $this->getMapper()->assign(new Admin_Model_Tourer_Entity(), $a_Row);
