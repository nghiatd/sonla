...
class Admin_Model_Content_Repository implements Zf_Model_RepositoryInterface, Admin_Model_Content_Interface
{
	protected $_data;
	 ...
	 * @author DungNT
	 * @since 04/01/2013
	 * @param Admin_Model_Content_DataMapper $the_o_data
	 * @return object
	 */
	public function setData($the_o_data)
	{
        ...
        if (!$the_o_data instanceof Admin_Model_Content_DataMapper) {

        	throw new Admin_Model_Content_Exception('Invalid data access object provided');
	 ...
	 * @author DungNT
	 * @since 04/01/2013
	 * @return Admin_Model_Content_DataMapper
	 */
	public function getData()
	{
		if (null === $this->_data) {

			$this->setData('Admin_Model_Content_DataMapper');
	...
	public function setMapper($the_o_Mapper)
	{
        ...
        if (!$the_o_Mapper instanceof Zf_Model_DataMapper) {
        	...
        	throw new Admin_Model_Content_Exception('Invalid data mapper provided');
	...
	public function getMapper()
	{
		if (null === $this->_mapper) {
            ...
            $this->setMapper('Admin_Model_Content_Mapper');
	...
	public function a_fGetCatById($the_i_Id)
	{
		try {
			...
			if( $a_Row && is_array($a_Row) ) {
				...
				$a_Content = $this->getMapper()->assign(new Admin_Model_Content_Entity(), $a_Row);
		...
		} catch (Admin_Model_Content_Exception $ex) {
	...
	public function a_fGetCatInfo($the_sz_Key, $the_sz_Value)
	{
		try {
			...
			if( $a_Row && is_array($a_Row) ) {
				...
				$a_Content = $this->getMapper()->assign(new Admin_Model_Content_Entity(), $a_Row)->__toArray();
	...
	public function o_fGetSelect($the_sz_Order = null, $the_a_FilterWhere = null)
	{
		...
		try {
			...
			$the_a_FilterWhere[$this->sz_fGetFieldMapper('category_id')] = $a_CatParentEntity['id'];
