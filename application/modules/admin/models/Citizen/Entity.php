<?php

class Admin_Model_Citizen_Entity extends Zf_Model_Entity
{
    protected $id;
    protected $code;
    protected $name_en;
    protected $name_vi;
    protected $image;
    protected $description_en;
    protected $description_vi;
    protected $content_en;
    protected $content_vi;
    protected $parent_id;
    protected $category_id;
    protected $alias;
    protected $createdDate;
    protected $delete;
    protected $deleteDate;
    protected $createdUserId;
    protected $sort;
    protected $status;
    protected $lastActivity;

    /**
     * @param int $the_i_Id
     */
    public function setId($the_i_Id) {
        $this->id = (int) $the_i_Id;
        return $this;
    }

    /**
     * @return int $id
     */
    public function getId() {
        return $this->id;
    }

    /**
     * @param string $the_sz_Code
     */
    public function setCode($the_sz_Code) {
        $this->code = (string) $the_sz_Code;
        return $this;
    }

    /**
     * @return string $code
     */
    public function getCode() {
        return $this->code;
    }

    /**
     * @param string $the_sz_NameEn
     */
    public function setName_en($the_sz_NameEn) {
        $this->name_en = (string) $the_sz_NameEn;
        return $this;
    }

    /**
     * @return string $name_en
     */
    public function getName_en() {
        return $this->name_en;
    }

    /**
     * @param string $the_sz_NameVi
     */
    public function setName_vi($the_sz_NameVi) {
        $this->name_vi = (string) $the_sz_NameVi;
        return $this;
    }

    /**
     * @return string $name_vi
     */
    public function getName_vi() {
        return $this->name_vi;
    }

    /**
     * @return string $image
     */
    public function getImage() {
        return $this->image;
    }

    /**
     * @param string $the_sz_image
     */
    public function setImage($the_sz_image) {
        $this->image = (string) $the_sz_image;
        return $this;
    }

    /**
     * @param string $the_sz_Alias
     */
    public function setAlias($the_sz_Alias) {
        $this->alias = (string) $the_sz_Alias;
        return $this;
    }

    /**
     * @return string $alias
     */
    public function getAlias() {
        return $this->alias;
    }

    /**
     * @param string $the_sz_DescriptionEn
     */
    public function setDescription_en($the_sz_DescriptionEn) {
        $this->description_en = (string) $the_sz_DescriptionEn;
        return $this;
    }

    /**
     * @return string $description_en
     */
    public function getDescription_en() {
        return $this->description_en;
    }

    /**
     * @param string $the_sz_DescriptionVi
     */
    public function setDescription_vi($the_sz_DescriptionVi) {
        $this->description_vi = (string) $the_sz_DescriptionVi;
        return $this;
    }

    /**
     * @return string $description_vi
     */
    public function getDescription_vi() {
        return $this->description_vi;
    }

    /**
     * @param string $the_sz_ContentVi
     */
    public function setContent_vi($the_sz_ContentVi) {
        $this->content_vi = (string) $the_sz_ContentVi;
        return $this;
    }

    /**
     * @return string $content_vi
     */
    public function getContent_vi() {
        return $this->content_vi;
    }

    /**
     * @param string $the_sz_ContentEn
     */
    public function setContent_en($the_sz_ContentEn) {
        $this->content_en = (string) $the_sz_ContentEn;
        return $this;
    }

    /**
     * @return string $content_en
     */
    public function getContent_en() {
        return $this->content_en;
    }

    /**
     * @param int $the_i_ParentId
     */
    public function setParent_id($the_i_ParentId) {
        $this->parent_id = (int) $the_i_ParentId;
        return $this;
    }

    /**
     * @return int $parent_id
     */
    public function getParent_id() {
        return $this->parent_id;
    }

    /**
     * @param int $the_i_CatId
     */
    public function setCategory_id($the_i_CatId) {
        $this->category_id = (int) $the_i_CatId;
        return $this;
    }

    /**
     * @return int $category_id
     */
    public function getCategory_id() {
        return $this->category_id;
    }

    /**
     * @param int $the_i_Sort
     */
    public function setSort($the_i_Sort) {
        $this->sort = (int) $the_i_Sort;
        return $this;
    }

    /**
     * @return int $sort
     */
    public function getSort() {
        return $this->sort;
    }

    /**
     * @param int $the_i_Status
     */
    public function setStatus($the_i_Status) {
        $this->status = (int) $the_i_Status;
        return $this;
    }

    /**
     * @return int $status
     */
    public function getStatus() {
        return $this->status;
    }

    /**
     * @param int $the_i_LastActivity
     */
    public function setLastActivity($the_i_LastActivity) {
        $this->lastActivity = (int) $the_i_LastActivity;
        return $this;
    }

    /**
     * @return int $lastActivity
     */
    public function getLastActivity() {
        return $this->lastActivity;
    }

    /**
     * @param int $the_i_CreatedDate
     */
    public function setCreatedDate($the_i_CreatedDate) {
        $this->createdDate = (int) $the_i_CreatedDate;
        return $this;
    }

    /**
     * @return int $creatDate
     */
    public function getCreatedDate() {
        return $this->createdDate;
    }

    /**
     * @param int $the_i_Delete
     */
    public function setDelete($the_i_Delete) {
        $this->delete = (int) $the_i_Delete;
        return $this;
    }

    /**
     * @return int $delete
     */
    public function getDelete() {
        return $this->delete;
    }

    /**
     * @param int $the_i_UserId
     */
    public function setCreatedUserId($the_i_UserId) {
        $this->createdUserId = (int) $the_i_UserId;
        return $this;
    }

    /**
     * @return int $createdUserId
     */
    public function getCreatedUserId() {
        return $this->createdUserId;
    }

    /**
     * @param int $the_i_DeleteDate
     */
    public function setDeleteDate($the_i_DeleteDate) {
        $this->deleteDate = (int) $the_i_DeleteDate;
        return $this;
    }

    /**
     * @return int $deleteDate
     */
    public function getDeleteDate() {
        return $this->deleteDate;
    }

}