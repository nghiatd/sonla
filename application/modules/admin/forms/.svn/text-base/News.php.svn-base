...
class Admin_Form_News extends Zend_Form
{
    ...
    public function init()
    {
        ...
        $o_NameEn->setLabel('ADMIN_NEWS_FORM_LABELS_NameEn')
        ...
        $o_NameVi->setLabel('ADMIN_NEWS_FORM_LABELS_NameVi')
	        ->addValidator('NotEmpty')
	        ->addValidator('Alnum', true, array('allowWhiteSpace' => true))
	        ->addValidator('stringLength', false, array(5, 50))
	        ->setRequired(true)
        ...
        $o_DescEn->setLabel('ADMIN_NEWS_FORM_LABELS_DescEn')
	        ->addValidator('Alnum', true, array('allowWhiteSpace' => true))
        ...
        $o_DescVi->setLabel('ADMIN_NEWS_FORM_LABELS_DescVi')
	        ->addValidator('Alnum', true, array('allowWhiteSpace' => true))
	        ->addValidator('stringLength', false, array(0, 255))
	        ->setRequired(false)
	        ->setAttrib('class', 'form-control')
        ...
        $o_ParentCat->setLabel('ADMIN_NEWS_FORM_LABELS_ParentCat')
        	  ->setAttrib('class', 'form-control')
        	  ->setRequired(true)
        	  ->setDecorators($this->_inputDecorator);

        $o_Sort = new Zend_Form_Element_Text('sort');
        $o_Sort->setLabel('ADMIN_NEWS_FORM_LABELS_Sort')
	        ->addValidator('NotEmpty')
	        ->addValidator('Digits')
        ...
        $o_Status->setLabel('ADMIN_NEWS_FORM_LABELS_Status')
	        ->addValidator('NotEmpty')
	        ->addValidator('stringLength', false, array(1, 5))
	        ->setRequired(true)
	        ->setValue(1)
    ...
    public function v_fCatFilterForm($the_sz_ActUrl, $the_a_AllParams)
    {
    	...
    	$o_Category = new Admin_Model_Categories_DataMapper();
    	$a_Categories = $o_Category->a_fGetCatListByParent(1, 1);
    	$a_CategoryData = array('' => $this->_translate->translate('ADMIN_TABLE_TITLE_NewsCatList'));
    	foreach($a_Categories as $key => $a_Category)
    	{
    		$a_CategoryData[$a_Category['CAT_Id']] = isset($a_Category['CAT_Name_'.$this->_locale]) ? $a_Category['CAT_Name_'.$this->_locale] : '' ;
    	}
    	if( $this->_locale == Zf_Util_Const::LANG_EN )
    	{
    		$o_NameEn = $this->getElement('name_en');
    		$o_NameEn->setLabel('ADMIN_NEWS_TABLE_TITLE_Name')
    				->removeValidator('NotEmpty')
		    		->setRequired(false)
		    		->setAttrib('class', 'form-control')
		    		->setAttrib('placeholder', $this->_translate->translate('ADMIN_NEWS_TABLE_TITLE_Name'))
		    		->setDecorators($this->_filterDecorators);

    		$o_DescEn = $this->getElement('description_en');
    		$o_DescEn->setLabel('ADMIN_CAT_TABLE_TITLE_Desc')
    				->setAttrib('class', 'form-control')
		    		->setAttrib('placeholder', $this->_translate->translate('ADMIN_NEWS_TABLE_TITLE_Desc'))
    				->setDecorators($this->_filterDecorators);
