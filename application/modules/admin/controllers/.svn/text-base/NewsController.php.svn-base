...
class Admin_NewsController extends Zend_Controller_Action
{
	...
	protected $_locale;
	protected $_o_NewsRepository;
	protected $_o_NewsForm;
    public function init()
    {
    	...
    	$this->_o_NewsRepository = new Admin_Model_News_Repository();

    	// Get category form
    	$this->_o_NewsForm = new Admin_Form_News();

    	$this->_a_AllParams = $a_AllParams;
    	$sz_Title = $this->_translate->translate('ADMIN_TITLE_PAGES_News');
    ...
    public function indexAction()
    {
    	...
    	$o_Select = $this->_o_NewsRepository->o_fGetSelect($sz_OrderStr, $a_FilterWhere);
    ...
    public function a_fGetTableConfig($the_sz_ActionName, $the_sz_Order)
    {
    	...
    	$sz_ConfirmDeleteCat = 'COMMON.v_fConfirmDelete(this.getAttribute(\'sz_Value\'), \'/admin/news/delete\', \'' . $this->_translate->translate('ADMIN_CAT_ALERT_MSG_DeleteCatConfirmTitle') . '\', \'' . $this->_translate->translate('ADMIN_CAT_ALERT_MSG_SureDeleteThisCat') . '\')';

    	// Set action url and param to filter form
    	$this->_o_NewsForm->v_fCatFilterForm($this->_sz_ActUrl, $this->_a_AllParams);

    	// Array table config
    	$a_TableConfig = array(
    			'setting' => array('filter' => true, 'multidelete' => true, 'locale' => $this->_locale),
    			'filterform' => $this->_o_NewsForm,
    			'columns' => array(
    					'name_' . $this->_locale		=> array('selected' => false, 'title' => $this->_translate->translate('ADMIN_NEWS_TABLE_TITLE_Name')),
    					'category_name' 				=> array('selected' => false, 'title' => $this->_translate->translate('ADMIN_TABLE_TITLE_NewsCatList')),
    			...
    			'mapper' => new Admin_Model_News_Mapper(),
    			'order' => $the_sz_Order,
    			...
    			'id' => 'id'
    					...
    					'controller' => array('name' => 'news', 'trans' => $this->_translate->translate('ADMIN_NAV_MENU_News')),
    					),
