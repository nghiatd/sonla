<?php
class Admin_TourerController extends Zend_Controller_Action
{
    /*  */
    protected $_a_AllParams = array();
    protected $_sz_ActUrl = '';
    protected $_locale;
    protected $_o_TourerRepository;
    protected $_o_TourerForm;
    ...
    public function init()
    {
    	...
    	// Get tourer model repository
    	$this->_o_TourerRepository = new Admin_Model_Tourer_Repository();

        // Get category model repository
    	$this->_o_CatRepository = new Admin_Model_Categories_Repository();

    	// Get tourer form
    	$this->_o_TourerForm = new Admin_Form_Tourer();

        // Get entity model Tourer
        $this->_o_TourerEntity = new Admin_Model_Tourer_Entity();

    	$this->_a_AllParams = $a_AllParams;
    	$sz_Title = $this->_translate->translate('ADMIN_TITLE_PAGES_Tourer');
    	$this->view->headTitle($sz_Title);
    	$this->view->selectedMenu = 'tourer';
    ...
    public function indexAction()
    {
    	...
    	$o_Select = $this->_o_TourerRepository->o_fGetSelect($sz_OrderStr, $a_FilterWhere);
    ...
    public function addAction()
    {
        ...
        $a_CategoryData = $o_CategoriesRepository->a_fGetParentCatForForm($sz_CatCode, $this->_translate->translate('ADMIN_TABLE_TITLE_TourerCatList'));

    	$this->_o_TourerForm->getElement('category_id')->setMultiOptions($a_CategoryData);

    	$this->view->o_CatForm = $this->_o_TourerForm;
    	...
    	if( $this->_request->isPost() ) {
            ...
            if( !$this->_o_TourerForm->isValid($this->_params) ) {

                return false;
            } else {
                // Set value to entity
                $this->_o_TourerEntity->setName_en( $this->_params['name_en'] )
                ...
                // Save data
                $this->_o_TourerRepository->v_fSave($this->_o_TourerEntity);
    ...
    public function  editAction()
    {
    	...
    	if( !$this->_request->isPost() )
        {
            ...
            $a_TourerInfo = $this->_o_TourerRepository->a_fGetTourerInfo( 'TOURER_id', $this->_request->getParam('id') );

            $this->_o_TourerForm->populate($a_TourerInfo);
            ...
            if( !$this->_o_TourerForm->isValid($this->_params) )
            {
                ...
                // Set value to entity
                $this->_o_TourerEntity->setId( $this->_params['id'] )
    ...
    public function deleteAction()
    {
    	...
    	$this->_o_TourerEntity->setId( $i_Id );

    	$b_Result = $this->_o_TourerRepository->b_fDelete( $this->_o_TourerEntity );
    	...
    	if( $b_Result )
    	{
            ...
            $o_Result->sz_Url = '/admin/tourer/' . $sz_ActionName . '/';

    	} else {

            $o_Result->sz_ResultMsg = $this->_translate->translate('ADMIN_TOURER_ALERT_MSG_CannotDeleteThis');
    ...
    public function a_fGetTableConfig($the_sz_ActionName, $the_sz_Order)
    {
    	...
    	$sz_ConfirmDeleteCat = 'COMMON.v_fConfirmDelete(this.getAttribute(\'sz_Value\'), \'/admin/tourer/delete\', \'index\',\'' . $this->_translate->translate('ADMIN_TOURER_ALERT_MSG_DeleteCatConfirmTitle') . '\', \'' . $this->_translate->translate('ADMIN_TOURER_ALERT_MSG_SureDeleteThisTourer') . '\')';

    	// Set action url and param to filter form
        $this->_o_TourerForm->v_fCatFilterForm($this->_sz_ActUrl, $this->getAllParams());

    	// Array table config
    	$a_TableConfig = array(
    			'setting' => array('filter' => true, 'multidelete' => true, 'locale' => $this->_locale),
    			'filterform' => $this->_o_TourerForm,
    			'columns' => array(
                                    'name_' . $this->_locale => array('selected' => false, 'title' => $this->_translate->translate('ADMIN_TOURER_TABLE_TITLE_Name')),
                                    'description_' . $this->_locale => array('selected' => false, 'title' => $this->_translate->translate('ADMIN_TOURER_TABLE_TITLE_Desc')),
                                    'category_id' => array('selected' => false, 'title' => $this->_translate->translate('ADMIN_TOURER_TABLE_TITLE_CAT'),'field' => 'CAT_Name_' . $this->_locale),
    			...
    			'mapper' => new Admin_Model_Tourer_Mapper(),
    			'order' => $the_sz_Order,
    			...
    			'url' => array(
                        ...
                        'info' => array(
                                ...
                                'controller' => array('name' => 'tourer', 'trans' => $this->_translate->translate('ADMIN_NAV_MENU_Tourer')),
