...
class Admin_CategoriesController extends Zend_Controller_Action
{
	...
	protected $_o_CatRepository;

	protected $_o_CatEntity;

	protected $_o_CatForm;
    ...
    public function init()
    {
        ...
        $this->_o_CatEntity = new Admin_Model_Categories_Entity();

        // Get category form
        $this->_o_CatForm = new Admin_Form_Categories();
        ...
        switch ($this->_sz_Action)
        {
        	...
        	case 'business':
        		...
        		break;
        	case 'citizen':
        		$sz_Title = $this->_translate->translate('ADMIN_TITLE_PAGES_CatCitizen');
        		break;
        		case 'add':
        	...
        	case 'tourer':
        		...
        		break;	
        	default:
        		$sz_Title = $this->_translate->translate('ADMIN_TITLE_PAGES_Categories');
				break;
    ...
    {
    	$this->v_fCategoriesManager(Zf_Util_Const::CAT_CODE_INTRO);
    }
    ...
    {
    	$this->v_fCategoriesManager(Zf_Util_Const::CAT_CODE_ORG);
    }
    ...
    {
    	$this->v_fCategoriesManager(Zf_Util_Const::CAT_CODE_NEWS);
    }
    ...
    {
    	$this->v_fCategoriesManager(Zf_Util_Const::CAT_CODE_BUSINESS);
    }
    ...
    {
    	$this->v_fCategoriesManager(Zf_Util_Const::CAT_CODE_CITIZEN);
    }
    ...
    {
    	$this->v_fCategoriesManager(Zf_Util_Const::CAT_CODE_TOURER);
    }
     ...
     */
    public function v_fCategoriesManager($the_sz_ActionName)
    {
    	...
    	$a_FilterWhere['CAT_Code'] = $the_sz_ActionName;

    	// Get select object from repository
    	$o_Select = $this->_o_CatRepository->o_fGetSelect($sz_OrderStr, $a_FilterWhere);
    ...
    public function a_fGetTableConfig($the_sz_CatCode, $the_sz_Order)
    {
    	...
    	$sz_ConfirmDeleteCat = 'COMMON.v_fConfirmDelete(this.getAttribute(\'sz_Value\'), \'/admin/categories/delete\', \'' . $the_sz_CatCode . '\', \'' . $this->_translate->translate('ADMIN_CAT_ALERT_MSG_DeleteCatConfirmTitle') . '\', \'' . $this->_translate->translate('ADMIN_CAT_ALERT_MSG_SureDeleteThisCat') . '\')';

    	// Set action url and param to filter form
    	$this->_o_CatForm->v_fCatFilterForm($this->_sz_ActUrl, $this->_a_AllParams);
    	...
    	$a_CatTrans = array(
    		...
    		Zf_Util_Const::CAT_CODE_CITIZEN => $this->_translate->translate('ADMIN_NAV_MENU_CatCitizen'),
    	...
    	$a_TableConfig = array(
    		...
    		'filterform' => $this->_o_CatForm,
    		'columns' => array(
    			'name_' . $this->_locale		=> array('selected' => false, 'title' => $this->_translate->translate('ADMIN_CAT_TABLE_TITLE_Name')),
    			'description_' . $this->_locale => array('selected' => false, 'title' => $this->_translate->translate('ADMIN_CAT_TABLE_TITLE_Desc')),
    		...
    		'mapper' => new Admin_Model_Categories_Mapper(),
    		'order' => $the_sz_Order,
    		...
    		'info' => array(
    			...
    			'controller' => array('name' => 'categories', 'trans' => $this->_translate->translate('ADMIN_NAV_MENU_Categories')),
    ...
    {
    	$a_ParentCat = $this->_o_CatRepository->a_fGetParentCatForForm();

    	$this->_o_CatForm->getElement('parent_id')->setMultiOptions($a_ParentCat);

    	$this->view->o_CatForm = $this->_o_CatForm;
    	...
    	if( $this->_request->isPost() ) {
	    	...
	    	if( !$this->_o_CatForm->isValid($this->_params) ) {
	    		...
	    		// Set value to entity
	    		$this->_o_CatEntity->setName_en( $this->_params['name_en'] )
	    		...
	    		// Save data
	    		$this->_o_CatRepository->v_fSave($this->_o_CatEntity);

	    		// Get parent category info by parent id
	    		$a_ParentCatInfo = $this->_o_CatRepository->a_fGetCatInfo( 'CAT_Id', $this->_request->getParam('parent_id') );
    ...
    public function editAction()
    {
    	...
    	if( !$this->_request->isPost() ) {
    		...
    		$a_CatInfo = $this->_o_CatRepository->a_fGetCatInfo( 'CAT_Id', $this->_request->getParam('id') );

    		$this->_o_CatForm->populate($a_CatInfo);
    		...
    		if( !$this->_o_CatForm->isValid($this->_params) ) {
    			...
    			// Set value to entity
    			$this->_o_CatEntity->setId( $this->_params['id'] )
    ...
    public function deleteAction()
    {
    	...
    	$this->_o_CatEntity->setId( $i_Id );

    	$b_Result = $this->_o_CatRepository->b_fDelete( $this->_o_CatEntity );
    ...
    public function multideleteAction()
    {
    	...
    	$b_Result = $this->_o_CatRepository->b_fMultiDelete( $a_IdsList );
